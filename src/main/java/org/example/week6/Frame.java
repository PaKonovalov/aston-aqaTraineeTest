...
public class Frame {
    ...
    public static void main(String[] args) {
        ...
        // определяем колличество вложенных окон на странице и выводим их колличество
        int iFrameSize = googleDriver.findElements(By.tagName("iframe")).size(); // запрашиваем колличество окон
        ...
        googleDriver.findElement(By.xpath("//input[@itemprop = 'query-input']")).sendKeys("Selenium Jobs");
        googleDriver.findElement(By.xpath("//button[@class = 'wpb_button wpb_btn-inverse btn']")).click();

        googleDriver.switchTo().defaultContent(); // переключаемся назад на основное окно

        googleDriver.findElement(By.xpath("//div[@class='hamburger-section container']/button")).click();
    }
}
